...
echo "blessed-chasm" > /etc/hostname
sed -i 's/127\.0\.0\.1.*localhost/127.0.0.1     blessed-chasm.localdomain blessed-chasm localhost/' /etc/hosts
hostname blessed-chasm
...
PASSWORD=$(mkpasswd AZIJoioBZBqzhvNXYXqN)
...
# Laravel Forge
ssh-rsa  worker@forge.laravel.com
...
debconf-set-selections <<< "mysql-community-server mysql-community-server/root-pass password rUxN9YFEgRKXHKEUVuPg"
debconf-set-selections <<< "mysql-community-server mysql-community-server/re-root-pass password rUxN9YFEgRKXHKEUVuPg"
...
mysql --user="root" --password="rUxN9YFEgRKXHKEUVuPg" -e "GRANT ALL ON *.* TO root@'165.22.63.28' IDENTIFIED BY 'rUxN9YFEgRKXHKEUVuPg';"
mysql --user="root" --password="rUxN9YFEgRKXHKEUVuPg" -e "GRANT ALL ON *.* TO root@'%' IDENTIFIED BY 'rUxN9YFEgRKXHKEUVuPg';"
service mysql restart

mysql --user="root" --password="rUxN9YFEgRKXHKEUVuPg" -e "CREATE USER 'forge'@'165.22.63.28' IDENTIFIED BY 'rUxN9YFEgRKXHKEUVuPg';"
mysql --user="root" --password="rUxN9YFEgRKXHKEUVuPg" -e "GRANT ALL ON *.* TO 'forge'@'165.22.63.28' IDENTIFIED BY 'rUxN9YFEgRKXHKEUVuPg' WITH GRANT OPTION;"
mysql --user="root" --password="rUxN9YFEgRKXHKEUVuPg" -e "GRANT ALL ON *.* TO 'forge'@'%' IDENTIFIED BY 'rUxN9YFEgRKXHKEUVuPg' WITH GRANT OPTION;"
mysql --user="root" --password="rUxN9YFEgRKXHKEUVuPg" -e "FLUSH PRIVILEGES;"

# Create The Initial Database If Specified

mysql --user="root" --password="rUxN9YFEgRKXHKEUVuPg" -e "CREATE DATABASE forge CHARACTER SET utf8 COLLATE utf8_unicode_ci;"
...
curl --insecure --data "event_id=30126139&server_id=307290&sudo_password=AZIJoioBZBqzhvNXYXqN&db_password=rUxN9YFEgRKXHKEUVuPg&recipe_id=22734" https://forge.laravel.com/provisioning/callback/app
