    ...
    <server-layout @mounted="serverMounted">
        <template slot="content">
            ...
            <confirm-modal
                ...
                :confirmText="
                    `Are you sure you want to delete the firewall rule ${deletingRule &&
                        deletingRule.name}?`
                "
            />
            <card
                ...
                v-if="server.provider === 'digital-ocean'"
            >
                <info>
...
export default {
    ...
    methods: {
        ...
        deleteRule() {
            axios
                .delete(
                    `/api/servers/${this.server.id}/firewall-rules/${this.deletingRule.id}`
                )
                .then(({ data: server }) => {
                    this.$root.servers = {
                        ...this.$root.servers,
        ...
        selectServer(checked, server) {
            if (checked) {
                this.form = {
                    ...this.form,
                    servers: [...this.form.servers, server]
                }
            } else {
                this.form = {
                    ...this.form,
        ...
        updateNetwork() {
            ...
            axios
                .patch(`/api/servers/${this.server.id}/network`, {
                    ...this.form,
                ...
                .catch(({ response }) => {
                    this.$root.flashMessage(
                        response.data.message || 'Failed updating network.',
                        'error'
                    )
                })
                .finally(() => {
        ...
        serverMounted() {
            this.form = {
                ...this.form,
                ports:
                    (this.server.friend_servers || []).length > 0
                        ? (this.server.friend_servers || [])[0].ports
                        : '',
                servers: (this.server.friend_servers || []).map(
                    server => server.friend_server_id
                )
            }
        },
        addRule() {
            ...
            axios
                .post(`/api/servers/${this.server.id}/firewall-rules`, {
                    ...this.firewallForm,
                    ...
                    if (response.status === 422) {
                        ...
                        if (invalidIpAddresses) {
                            this.errors = {
                                ...this.errors,
                                from: [
                                    'Some of the ip addresses are invalid. Please check again.'
                                ]
                            }
                        }
                    }
