    ...
    <component :is="wrapper" @mounted="serverMounted">
        <template slot="content">
            ...
            <confirm-modal
                ...
                :confirmText="
                    `Are you sure you want to delete the ssh key ${selectedKey &&
                        selectedKey.name}?`
                "
            />
...
export default {
    data() {
        return {
            ...
            wrapper:
                this.$route.name === 'account.ssh-keys'
                    ? 'account-layout'
                    : 'server-layout'
        }
    },
    ...
    methods: {
            ...
            this.submitForm()
                ...
                .catch(({ response }) => {
                    this.$root.flashMessage(
                        response.data.message || 'Failed adding key.',
                        'error'
                    )
                })
        },
        deleteKey() {
            ...
            axios
                ...
                .catch(({ response }) => {
                    this.$root.flashMessage(
                        response.data.message || 'Failed deleting SSH key.',
                        'error'
                    )
                })
                .finally(() => {
