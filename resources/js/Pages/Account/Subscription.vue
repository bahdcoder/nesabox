        ...
        <template slot="content">
            ...
            <card title="Update your nesabox plan" class="mb-6">
                <div v-if="!updating" class="w-full flex flex-wrap">
                    <div v-for="plan in plans" :key="plan.key" class="w-full sm:w-1/2 lg:w-1/3 px-2 mt-4 md:mt-0">
                        <div class="bg-white shadow rounded overflow-hidden">
                            ...
                            <div class="bg-white px-4 py-4">
                                <div v-for="feature in plan.features" :key="feature.name" class="mt-4">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-2" :class="{ 'font-medium': feature.bold }">
                                            {{ feature.name }}
                                        </div>
                                    </div>
                                </div>
                        ...
                        <p v-if="subscription.plan === plan.key" class="mt-4 text-center font-medium text-lg">Your current plan</p>

                        <component
                            v-else
                            :full="true"
                            class="mt-4"
                            :is="getPlanLabel(plan).match(/Downgrade/) ? 'red-button' : 'v-button'"
                ...
                axios
                    ...
                    .catch(({ response }) => {
                        if (response.data && response.data.message ) {
                            ...
                            this.$root.flashMessage('Failed to cancel subscription.', 'error')
                        }
                    })
                    .finally(() => {
                ...
                axios
                    ...
                    .catch(({ response }) => {
                        if (response.data && response.data.message ) {
                            ...
                            this.$root.flashMessage('Failed to update subscription plan.', 'error')
                        }
                    })
                    .finally(() => {
                        this.updating = false
