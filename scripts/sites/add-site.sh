...
# Create config file for nesabox app
cat > /etc/nginx/sites-available/$NESABOX_SITE_NAME << EOF
server {
    listen 80;
    server_name $NESABOX_SITE_NAME;
...
ln -s /etc/nginx/sites-available/$NESABOX_SITE_NAME /etc/nginx/sites-enabled/
